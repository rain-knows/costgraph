...
`parse_cost_question` 输出成本意图、零件、目标期间、对比基准期和 `report_style`；显式期间仍由服务端解析器覆盖模型建议。`generate_cost_analysis` 的输入固定为零件、期间、确定性计算结果和当前状态栏；周期对比时同一计算结果参数附带基准期三视图。确定性结果中的 Decimal 在发送模型前按原精度转为 JSON 字符串。Report 工具按成本数据契约接受批次 `datetime`，并由 Pydantic 在公开 JSON 中序列化为时间字符串。回归测试覆盖严格方法签名和 Decimal 序列化，防止图节点与生产 Adapter 的调用参数再次漂移。
