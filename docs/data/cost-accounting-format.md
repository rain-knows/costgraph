...
- 累计结果、三视图、单位成本、比例和占比均为服务端派生值，不写回成本事实表；发布事务将其写入带快照和规则版本的只读查询投影，以避免请求时重复卷积。
...
样例核算对象固定为汽车装饰件工厂。规范样例包含 100 个产成品 SKU，覆盖 `2025-01` 至 `2026-12` 连续 24 个自然月；每月最少 10 个产成品批次。10 个核心 SKU 每月都有批次，其余 SKU 在两个年度的相同自然月重复出现，因此全部 SKU 至少可做同比，核心 SKU 还可做连续环比。批次共同使用多种通用原料、外购件和在制品零件定义，但每次生产使用独立事件批次，保持实际领用 DAG 可审计。运行 `backend/scripts/generate_sample_cost_data.py` 可确定性重建扩展样例，同时保留下述黄金批次。

原有 3 个产成品、2 个期间、6 个最终批次继续作为固定黄金子集。黄金批次追溯链包含原材料与外购卡扣两类投入，以及“注塑成型 → 火焰处理与表皮包覆 → 卡扣压装与门板总成装配”三道连续生产工序；同时覆盖多投入、部分领用、批次分流、零/非零不良品和三项制造后费用。
